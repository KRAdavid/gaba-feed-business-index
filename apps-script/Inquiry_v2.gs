...
/** Run once in the editor. Creates the sheet, requests permissions, and sends a test mail. */
...
function doPost(e) {
  try {
    ...
    return gabaInquiryResponseV2_(false, '문의 처리 중 오류가 발생했습니다. 메일 앱 전송을 이용해 주세요.', '');
  }
}
...
function gabaInquiryProcessV2_(params) {
  ...
  const data = {
    ...
    sourcePage: gabaInquiryValueV2_(params, '접수페이지') || gabaInquiryValueV2_(params, 'source_page'),
    clientTimestamp: gabaInquiryValueV2_(params, '접수시각') || gabaInquiryValueV2_(params, 'client_timestamp'),
    formVersion: gabaInquiryValueV2_(params, 'form_version') || GABA_INQUIRY_CFG_V2.FORM_VERSION
  };

  const validationError = gabaInquiryValidateV2_(data);
  if (validationError) return gabaInquiryResponseV2_(false, validationError, '');
  if (!gabaInquiryAllowV2_(data.email, data.company)) {
    return gabaInquiryResponseV2_(false, '연속 제출이 감지되었습니다. 30초 후 다시 시도해 주세요.', '');
  }

  const now = new Date();
  const receivedAt = Utilities.formatDate(now, GABA_INQUIRY_CFG_V2.TZ, 'yyyy-MM-dd HH:mm:ss');
  const inquiryId = 'INQ-' + Utilities.formatDate(now, GABA_INQUIRY_CFG_V2.TZ, 'yyyyMMdd-HHmmss') + '-' +
  ...
  try {
    if (MailApp.getRemainingDailyQuota() < 1) throw new Error('MailApp daily quota exhausted');
    ...
    try {
      MailApp.sendEmail({
        ...
        body: data.contactName + '님, 문의가 접수되었습니다.\n문의번호: ' + inquiryId +
  ...
  if (mailStatus !== 'sent') {
    return gabaInquiryResponseV2_(false,
      '문의는 기록되었으나 이메일 발송에 실패했습니다. 메일 앱으로 보내기를 눌러 주세요.', inquiryId,
  ...
  return gabaInquiryResponseV2_(true, '문의가 정상 접수되었습니다.' + warning, inquiryId, {
...
function gabaInquiryValidateV2_(data) {
  const required = [
    ['회사·기관명', data.company], ['담당자명', data.contactName], ['이메일', data.email],
    ['국가·지역', data.countryRegion], ['협업 유형', data.collaborationType], ['주요 축종', data.species],
    ['현재 과제', data.currentChallenges], ['현재 준비 단계', data.preparationStage],
    ['희망 시작 시기', data.desiredStart], ['상세 요청사항', data.detailedRequest],
...
function gabaInquiryValueV2_(params, name) {
  ...
  return list.map(function(value) { return String(value == null ? '' : value).trim(); })
    .filter(Boolean).join(', ').slice(0, GABA_INQUIRY_CFG_V2.MAX_FIELD_LENGTH);
}

function gabaInquiryAllowV2_(email, company) {
  const cache = CacheService.getScriptCache();
  const digest = Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256,
    String(email).toLowerCase() + '|' + String(company).toLowerCase(), Utilities.Charset.UTF_8);
...
function gabaInquirySheetV2_() {
  ...
  if (!sheet) sheet = ss.insertSheet(GABA_INQUIRY_CFG_V2.SHEET_NAME);
  if (sheet.getLastRow() === 0) {
    sheet.getRange(1, 1, 1, GABA_INQUIRY_HEADERS_V2.length).setValues([GABA_INQUIRY_HEADERS_V2]);
    sheet.setFrozenRows(1);
    sheet.getRange(1, 1, 1, GABA_INQUIRY_HEADERS_V2.length).setFontWeight('bold');
  }
  return sheet;
}

function gabaInquiryAppendV2_(inquiryId, receivedAt, mailStatus, mailError, data) {
...
function gabaInquiryRowsV2_(inquiryId, receivedAt, data) {
  return [
    ['문의번호', inquiryId], ['접수시각', receivedAt], ['회사·기관명', data.company],
    ['담당자명', data.contactName], ['직책·부서', data.departmentTitle], ['이메일', data.email],
    ['연락처', data.phone], ['국가·지역', data.countryRegion], ['협업 유형', data.collaborationType],
    ['선택 제품', data.selectedProduct], ['선택 규격·형태', data.selectedSpecification],
    ['주문 가이드 경로', data.orderGuidePath], ['주요 축종', data.species],
    ['사육규모·사료물량', data.farmFeedVolume], ['현재 해결 과제', data.currentChallenges],
    ['현재 준비 단계', data.preparationStage], ['희망 시작 시기', data.desiredStart],
    ['예상 샘플·초도 물량', data.sampleInitialVolume], ['예상 연간 물량', data.annualVolume],
    ['핵심 평가지표', data.evaluationKpis], ['상세 요청사항', data.detailedRequest],
    ['개인정보 전송동의', data.consent], ['접수 페이지', data.sourcePage],
...
function gabaInquiryPlainBodyV2_(inquiryId, receivedAt, data) {
  return gabaInquiryRowsV2_(inquiryId, receivedAt, data)
    .map(function(row) { return row[0] + ': ' + (row[1] || '미입력'); }).join('\n\n');
}

function gabaInquiryHtmlBodyV2_(inquiryId, receivedAt, data) {
  const rows = gabaInquiryRowsV2_(inquiryId, receivedAt, data).map(function(row) {
    return '<tr><th style="padding:9px 12px;text-align:left;background:#f2f7f4;border:1px solid #dce7e1;white-space:nowrap">' +
      gabaInquiryEscapeHtmlV2_(row[0]) + '</th><td style="padding:9px 12px;border:1px solid #dce7e1;line-height:1.55">' +
...
function gabaInquiryResponseV2_(ok, message, inquiryId, extra) {
  ...
  const html = '<!doctype html><html><head><meta charset="utf-8"><title>Inquiry response</title></head>' +
    ...
    '<p style="font-family:sans-serif">' + gabaInquiryEscapeHtmlV2_(message) + '</p></body></html>';
